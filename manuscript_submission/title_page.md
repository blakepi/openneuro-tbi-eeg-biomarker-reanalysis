# Title Page

## Title
**Leakage-Safe EEG Biomarker Analysis After Mild Traumatic Brain Injury: A Preregistered OpenNeuro Reanalysis and Replication Feasibility Study**

## Running title
Leakage-safe EEG biomarkers after mTBI

## Author
Gregory Blake Pierpoint

## Affiliation
Macon and Joan Brock Virginia Health Sciences, Eastern Virginia Medical School at Old Dominion
University, Norfolk, Virginia, USA

## Corresponding author
Gregory Blake Pierpoint
Macon and Joan Brock Virginia Health Sciences, Eastern Virginia Medical School at Old Dominion
University, Norfolk, Virginia, USA
Email: pierpogb@odu.edu

## ORCID
Gregory Blake Pierpoint — https://orcid.org/0000-0001-8288-8549

## Word counts
- Abstract: 233 words
- Main text (Introduction–Conclusions): ~3,300 words
- Figures: 7 main + 5 supplementary
- Tables: 5 main + 11 supplementary
- References: 26

## Keywords
mild traumatic brain injury; post-concussive symptoms; electroencephalography; event-related
potentials; aperiodic activity; prognostic biomarkers; reproducibility; data leakage;
preregistration; OpenNeuro

## Preregistration / analysis-plan availability
The analysis plan was locked before any feature–outcome relationship was examined
(`final_locked_analysis_plan.md`, included as Supplement and version-controlled in the code
repository, where the pre-modeling timestamp is recorded in the commit history).

## Funding
No funding was received for this work.

## Conflicts of interest
The author declares no conflicts of interest.

## Authorship
Single-author manuscript.

## Data and code availability
Public OpenNeuro datasets (ds003522, ds005114, ds003523, ds003490); analysis code at https://github.com/blakepi/openneuro-tbi-eeg-biomarker-reanalysis (MIT License) and archived on Zenodo at https://doi.org/10.5281/zenodo.20682573 (DOI: 10.5281/zenodo.20682573). Raw EEG is not redistributed.
